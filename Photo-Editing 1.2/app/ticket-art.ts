...
export function drawPhoto(ctx: CanvasRenderingContext2D, image: HTMLImageElement, x: number, y: number, w: number, h: number, fit: 'cover' | 'contain' = 'cover', positionX = 50, positionY = 50) {
  const scale = (fit === 'cover' ? Math.max : Math.min)(w / image.naturalWidth, h / image.naturalHeight);
  const width = image.naturalWidth * scale, height = image.naturalHeight * scale;
  ctx.save(); ctx.beginPath(); ctx.rect(x, y, w, h); ctx.clip();
  ctx.drawImage(image, x + (w - width) * (positionX / 100), y + (h - height) * (positionY / 100), width, height);
  ctx.restore();
}
