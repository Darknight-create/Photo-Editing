...
const collageCopy = {
  zh: {
    ...
    collageMoveMode: '移动图片',
    collagePatchMode: '单向贴片',
    collageExchangeMode: '双向交换',
    collageExchangeHelp: '在当前画幅框选后，上下两张照片会交换同一位置的正方形区域。',
  },
  en: {
    ...
    collageMoveMode: 'move image',
    collagePatchMode: 'one-way patch',
    collageExchangeMode: 'two-way swap',
    collageExchangeHelp: 'Draw on the selected frame to exchange the same square area between both photos.',
  },
} as const;
...
function GalleryModule({ t, language, setLanguage, active }: { t: AppCopy; language: Language; setLanguage: (language: Language) => void; active: boolean }) {
...
function CollageModule({ t, language, setLanguage }: { t: AppCopy; language: Language; setLanguage: (language: Language) => void }) {
  ...
  const cropPane = async (pane: CollagePane, crop: PatchSelection) => {
    const source = images[pane];
  ...
  const createPatch = async (crop: PatchSelection) => {
    ...
    const sourcePatch = await cropPane(crop.pane, crop);
    if (!sourcePatch) return;
    const id = Date.now();
    const nextPatches: CollagePatch[] = [{ ...crop, id, src: sourcePatch, targetPane, centerX, centerY }];
    if (exchangeMode && images[targetPane].src) {
      const targetPatch = await cropPane(targetPane, crop);
      if (targetPatch) nextPatches.push({ ...crop, pane: targetPane, id: id + 1, src: targetPatch, targetPane: crop.pane, centerX, centerY });
    }
    setPatches((current) => [...current, ...nextPatches]);
    ...
    <>
      ...
      <aside className="inspector collage-inspector">
        ...
        <section className="inspector-section">
          ...
          <p className="section-copy">{exchangeMode ? t.collageExchangeHelp : selectionMode ? t.collagePatchActive : t.collagePatchHelp}</p>
          <div className="collage-mode-switch" role="group" aria-label={t.collagePatch}>
            <button className={!selectionMode ? 'active' : ''} type="button" onClick={() => { setSelectionMode(false); setExchangeMode(false); setSelection(null); }}>{t.collageMoveMode}</button>
            <button className={selectionMode && !exchangeMode ? 'active' : ''} type="button" disabled={!images[selectedPane].src} onClick={() => { setSelectionMode(true); setExchangeMode(false); setSelection(null); setSelectedPatchId(null); }}>{t.collagePatchMode}</button>
            <button className={selectionMode && exchangeMode ? 'active' : ''} type="button" disabled={readyCount < 2} onClick={() => { setSelectionMode(true); setExchangeMode(true); setSelection(null); setSelectedPatchId(null); }}>{t.collageExchangeMode}</button>
          </div>
          {selectionMode && <p className="collage-mode-note">{exchangeMode ? t.collageExchangeHelp : t.collagePatchActive}</p>}
...
export default function Home() {
  ...
  return (
    <main className="editor-shell">
      ...
      <div className={`module-view ${activeModule === 'grid' ? 'active' : ''}`}>
