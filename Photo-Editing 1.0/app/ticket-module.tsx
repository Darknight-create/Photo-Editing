...
export default function TicketModule({ language, setLanguage }: { language: 'zh' | 'en'; setLanguage: (value: 'zh' | 'en') => void }) {
  ...
  useEffect(() => {
    ...
    ctx.restore();
    drawPhoto(ctx, image, 0, ticketFirst ? H : 0, W, H, fit);
  }, [image, background, style, text, ticketFirst, fit]);
  ...
  const upload = async (file?: File) => {
    ...
    try {
      ...
      setBackground(centerColor(photo)); setImage(photo);
  ...
  const download = async (combined: boolean) => {
    ...
    try {
      ...
      if (combined) drawPhoto(ctx, art.image, 0, ticketFirst ? H : 0, W, H, fit);
  ...
  return <>
    <section className="workspace ticket-workspace">
      ...
      <div className="ticket-area">
        ...
        <div className="ticket-preview" aria-busy={loading} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); void upload(event.dataTransfer.files[0]); }}>
          {image ? <>
